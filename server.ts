...
// Helper to synthesize a procedural medieval fantasy WAV buffer
function synthesizeFantasyWav(prompt: string): { audioBase64: string; title: string; genre: string } {
  ...
  let bpm = 112;
  let scale = [293.66, 329.63, 349.23, 392.00, 440.00, 493.88, 523.25, 587.33]; // D Dorian
  let bassNotes = [146.83, 174.61, 196.00, 220.00];
  let title = "Tavern Hearth & Bard's Jig";
  let genre = "Celtic Fantasy Folk";

  if (lower.includes("dungeon") || lower.includes("dark") || lower.includes("catacomb") || lower.includes("shadow")) {
    bpm = 72;
    scale = [146.83, 155.56, 174.61, 196.00, 220.00, 233.08, 261.63, 293.66]; // D Phrygian
    bassNotes = [73.42, 77.78, 87.31, 110.00];
    title = "Dungeon Catacombs & Dark Depths";
    genre = "Dark Ambient Dungeon Theme";
  } else if (lower.includes("boss") || lower.includes("battle") || lower.includes("dragon") || lower.includes("fight")) {
    bpm = 130;
    scale = [130.81, 155.56, 174.61, 196.00, 207.65, 261.63, 293.66, 392.00]; // C Minor
    bassNotes = [65.41, 77.78, 87.31, 98.00];
    title = "Dragon Boss Battle & War March";
    genre = "Epic Orchestral War Theme";
  } else if (lower.includes("elf") || lower.includes("forest") || lower.includes("grove") || lower.includes("mystic")) {
    bpm = 86;
    scale = [329.63, 392.00, 440.00, 493.88, 587.33, 659.25, 783.99, 880.00]; // E Pentatonic
    bassNotes = [164.81, 196.00, 220.00, 246.94];
    title = "Mystic Elven Grove & Feywild";
    genre = "Ethereal Fantasy Soundtrack";
  }

  const stepDur = 60 / bpm / 2;
  const leadPattern = [0, 2, 4, 3, 2, 0, 4, 6, 5, 4, 2, 3, 2, 0, 2, 4];

  for (let s = 0; s < leadPattern.length * 4; s++) {
    const stepIdx = s % leadPattern.length;
    const startTime = s * stepDur;
    if (startTime >= seconds) break;

    const startSample = Math.floor(startTime * sampleRate);
    const noteIdx = leadPattern[stepIdx];
    const freq = scale[noteIdx % scale.length];
    const durSamples = Math.floor(stepDur * 1.6 * sampleRate);
    ...
    for (let i = 0; i < durSamples && startSample + i < numSamples; i++) {
      ...
      buffer[startSample + i] += wave * env * 0.28;
    }

    // Bass Note
    if (stepIdx % 4 === 0) {
      const bassFreq = bassNotes[Math.floor(stepIdx / 4) % bassNotes.length];
      const bassDur = Math.floor(stepDur * 3.8 * sampleRate);
      for (let i = 0; i < bassDur && startSample + i < numSamples; i++) {
        const t = i / sampleRate;
        const env = Math.exp(-t * 1.5);
        const wave = Math.sin(2 * Math.PI * bassFreq * t) + 0.4 * Math.sin(2 * Math.PI * (bassFreq * 1.01) * t);
        buffer[startSample + i] += wave * env * 0.32;
      }
    }

    // Drum percussion
    if (stepIdx % 2 === 0) {
      const drumDur = Math.floor(sampleRate * 0.12);
      for (let i = 0; i < drumDur && startSample + i < numSamples; i++) {
        const t = i / sampleRate;
        const f = 130 * Math.exp(-t * 22);
        buffer[startSample + i] += Math.sin(2 * Math.PI * f * t) * Math.exp(-t * 18) * 0.3;
      }
    }
  }

  // Convert buffer to 16-bit PCM WAV
  const wavBytes = Buffer.alloc(44 + numSamples * 2);
  wavBytes.write("RIFF", 0);
  wavBytes.writeUInt32LE(36 + numSamples * 2, 4);
  ...
  wavBytes.writeUInt32LE(numSamples * 2, 40);

  let offset = 44;
  for (let i = 0; i < numSamples; i++, offset += 2) {
    const s = Math.max(-1, Math.min(1, buffer[i]));
    wavBytes.writeInt16LE(s < 0 ? s * 0x8000 : s * 0x7fff, offset);
  }
...
// Music Generation Endpoint (Supports Lyria 3 + Procedural Fantasy Audio Engine)
app.post("/api/generate-music", async (req, res) => {
  const { prompt = "Cinematic fantasy adventure soundtrack with tavern lute and atmospheric flutes", trackType = "clip" } = req.body || {};
  const ai = getGeminiClient();

  if (ai) {
    const modelName = trackType === "full" ? "lyria-3-pro-preview" : "lyria-3-clip-preview";
    try {
      ...
      if (audioBase64) {
        return res.json({
          ...
          mimeType,
          lyrics,
          model: `Lyria 3 (${modelName})`,
          isLyria: true,
          duration: trackType === "full" ? "full" : "30s",
        });
      }
    } catch {
      // Gracefully fall back to procedural audio synthesis
    }
  }

  // Generate tailored procedural fantasy audio
  const synth = synthesizeFantasyWav(prompt);
  return res.json({
    ...
    title: synth.title,
    genre: synth.genre,
    model: "Fantasy Melodic Synthesizer",
    isLyria: false,
    duration: "15s loop",
    note: "Procedural fantasy soundtrack synthesized successfully."
  });
});
